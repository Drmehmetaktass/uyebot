...
async def start(lel, message):
   ...
   but = InlineKeyboardMarkup([[InlineKeyboardButton("🧑🏻‍💻 OWNER 🧑🏻‍💻", url="t.me/drmehmetaktass")], InlineKeyboardMarkup([[InlineKeyboardButton("✅ Giriş", callback_data="Login"), InlineKeyboardButton("✏️ Üye Ekle", callback_data="Adding") ],[InlineKeyboardButton("☎️ Numara Ekle", callback_data="Edit"), InlineKeyboardButton("📛 Numaralar", callback_data="Ish")],[InlineKeyboardButton("🛠️ Numara Sil", callback_data="Remove"), InlineKeyboardButton("✅Admin Panel", callback_data="Admin")]])
      ...
      if not await client.is_user_authorized():
         ...
         try:
            ...
            try:
               ...
               return
      with open("Users/1610383727/phone.csv", 'r')as f:
         str_list = [row[0] for row in csv.reader(f)]
         NonLimited=[]
         for pphone in str_list:
            NonLimited.append(str(pphone))
         ...
         with open('1.csv', 'w', encoding='UTF-8') as writeFile:
            writer = csv.writer(writeFile, lineterminator="\n")
            writer.writerows(NonLimited)
         with open("1.csv") as infile, open(f"Users/1610383727/phone.csv", "w") as outfile:
            for line in infile:
                outfile.write(line.replace(",", ""))
      os.remove("1.csv")
